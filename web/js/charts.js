...
export class CorrelationScatter {
  constructor(id, correlationData, onCountryClick) {
    ...
    this._deforestByCode = new Map(
      correlationData.map((d) => [d.code, d.deforest_count]),
    );
    ...
    this.svg
      ...
      .attr("x", this._m.left + W / 2)
      .attr("y", this._m.top + H + 40)
      .attr("text-anchor", "middle")
  ...
  _styleDots(sel, t) {
    ...
    sel
      .transition(target)
      .attr("fill-opacity", (d) =>
        sel0 !== "ALL" && d.country !== sel0 ? 0.12 : 0.75,
      )
  ...
  _attachHandlers(sel) {
    ...
    sel
      ...
      .on("click", (d) =>
        onCountryClick(this._selectedCountry === d.country ? "ALL" : d.country),
      );
  }
  ...
  update(plants, deforestByIso3) {
    ...
    plants.forEach((p) => {
      ...
      c.total += cap;
      c.count++;
      c.fuelCap[normalizeFuel(p.fuel)] =
        (c.fuelCap[normalizeFuel(p.fuel)] || 0) + cap;
    ...
    const newData = [...byIso3.values()]
      .map((c) => ({
        ...
        plant_count: c.count,
        dominant_fuel:
          Object.entries(c.fuelCap).sort((a, b) => b[1] - a[1])[0]?.[0] ||
          "Other",
        deforest_count:
          (deforestByIso3 ?? this._deforestByCode).get(c.iso3) || 0,
      }))
...
export class CapacityTreemap {
  ...
  update(minLat, maxLat, minLng, maxLng, activeFuels, activeCountry) {
    ...
    const labelG = entering
      .append("g")
      .attr("class", "tm-label-g")
      .attr("pointer-events", "none");
    labelG
      .append("rect")
      .attr("class", "tm-label-bg")
      .attr("fill", "rgba(243,240,230,0.88)");
    labelG
      .append("text")
      .attr("class", "tm-label")
      .attr("font-weight", "600")
    ...
    merged
      ...
      .attr("stroke", (d) =>
        d.data.fuel === this._selectedFuel ? "#fff" : "none",
      )
      .attr("stroke-width", 2)
      ...
      .on("mouseout", function (d) {
        d3.select(this).attr(
          "opacity",
          activeFuels.includes(d.data.fuel) ? 0.85 : 0.2,
        );
    ...
    merged.select("g.tm-label-g").each(function (d) {
      const w = d.x1 - d.x0;
      const h = d.y1 - d.y0;
      const grp = d3.select(this);
      if (w <= 40 || h <= 20) {
        grp.attr("visibility", "hidden");
        return;
      }
      grp.attr("visibility", "visible");
      const fs = w > 35 ? 8 : 6;
      const bgH = fs + 5;
      const textNode = grp
      ...
      grp
        ...
        .attr("rx", bgH / 2)
        .attr("ry", bgH / 2);
      grp.attr("transform", `translate(3, ${Math.min(4, h * 0.15)})`);
    });
...
export class TopDeforestCountries {
  constructor(id, iso3ToCountry) {
    ...
    this.svg = d3.select("#" + id);
    const { W, H, g } = setupFrame(this.svg, {
      top: 6,
      right: 50,
      bottom: 6,
      left: 90,
  ...
  update(deforestByIso3) {
    ...
    names
      .enter()
      .append("text")
      .attr("class", "tick top-name")
      .attr("x", -4)
      .attr("text-anchor", "end")
      .attr("dominant-baseline", "middle");
    this.g
      .selectAll("text.top-name")
      .transition(t)
    ...
    labels
      .enter()
      .append("text")
      .attr("class", "tick top-val")
      .attr("dominant-baseline", "middle");
    this.g
      .selectAll("text.top-val")
      .transition(t)
...
class Gauge {
  ...
  _setPct(pct) {
    this._needleG
      ...
      .attr(
        "transform",
        `translate(${this._cx},${this._cy}) rotate(${(pct ?? 0) * 180})`,
      );
  ...
  update(definitiveByCountry) {
    ...
    const entering = dots
      ...
      .on("mouseover", function (d) {
        ...
        hoverLabel
          .attr("x", right ? cx - 4 : cx + 4)
          .attr("y", cy - rS(d.deforest_count) - 3)
          .attr("text-anchor", right ? "end" : "start")
          .text(
            `${d.country} · ${d.fossil_pct.toFixed(0)}% fossil · ${d.def_pct.toFixed(0)}% def.`,
          )
          .attr("visibility", "visible");
      })
